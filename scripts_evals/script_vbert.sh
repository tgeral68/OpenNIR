...
#for fold in {2..2}; do
#CUDA_VISIBLE_DEVICES=2 python -m onir.bin.pipeline pipeline=jesus modelspace=try1-ft config/msmarco config/covidj/test${fold} config/vanilla_bert $BERT_MODEL_PARAMS pipeline.onlytest=true ranker.add_runscore=True data_dir=../data pipeline.test=true pipeline.finetune=true trainer.pipeline=msmarco_train_bm25_k1-0.82_b-0.68.100_mspairs >vbert_$fold.out 2>vbert_$fold.err
#done
...
#for fold in {2..2}; do
#    CUDA_VISIBLE_DEVICES=2 python -m onir.bin.pipeline pipeline=jesus modelspace=try1-ft config/covidj/fold${fold} config/vanilla_bert $BERT_MODEL_PARAMS ranker.add_runscore=True data_dir=../data pipeline.test=true pipeline.finetune=true trainer.pipeline=${prev_models[0]} >vbert_$fold.retrained.out 2>vbert_$fold.retrained.err
#done
...
for fold in {2..2}; do
CUDA_VISIBLE_DEVICES=2 python -m onir.bin.pipeline pipeline=jesus modelspace=try1-ft config/covidj/fold${fold} config/msmarco/judgeddev config/vanilla_bert $BERT_MODEL_PARAMS pipeline.onlytest=true ranker.add_runscore=True data_dir=../data pipeline.test=true pipeline.finetune=true trainer.pipeline=${prev_models[2]} >vbert_$fold.out 2>vbert_$fold.err
done
