...
#for fold in {2..2}; do
#CUDA_VISIBLE_DEVICES=0 python -m onir.bin.pipeline pipeline=jesus modelspace=try1-ft vocab.source=glove vocab.variant=cc-42b-300d config/msmarco config/covidj/test${fold} ranker=knrm pipeline.onlytest=true ranker.add_runscore=True data_dir=../data pipeline.test=true pipeline.finetune=true trainer.pipeline=msmarco_train_bm25_k1-0.82_b-0.68.100_mspairs >knrm_$fold.out 2>knrm_$fold.err
#done
...
#for fold in {2..2}; do
#    CUDA_VISIBLE_DEVICES=0 python -m onir.bin.pipeline pipeline=jesus modelspace=try1-ft vocab.source=glove vocab.variant=cc-42b-300d config/covidj/fold${fold} ranker=knrm ranker.add_runscore=True data_dir=../data pipeline.test=true pipeline.finetune=true trainer.pipeline=${prev_models[0]} >knrm_$fold.retrained.out 2>knrm_$fold.retrained.err
#done
...
for fold in {2..2}; do
CUDA_VISIBLE_DEVICES=1 python -m onir.bin.pipeline pipeline=jesus modelspace=try1-ft vocab.source=glove vocab.variant=cc-42b-300d config/covidj/fold${fold} config/msmarco/judgeddev ranker=knrm pipeline.onlytest=true ranker.add_runscore=True data_dir=../data pipeline.test=true pipeline.finetune=true trainer.pipeline=${prev_models[2]} >knrm_$fold.out 2>knrm_$fold.err
done
