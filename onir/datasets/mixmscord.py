...
import pandas as pd
import numpy as np
from onir.datasets import covid, msmarco
import copy
...
"""
Dataset MSMarco + Cord19
"""

@datasets.register('mixmscord')
class MixmscordDataset(datasets.IndexBackedDataset):
...
http://www.msmarco.org/dataset.aspx"""
    ...
    def pair_iter(self, fields, pos_source='intersect', neg_source='run', sampling='query', pos_minrel=1, unjudged_rel=0, num_neg=1, random=None, inf=False):
        ...
        counter = 0
        no_ms, no_cord=False,False # collections finished
        while True: # each batch 16 samples 
            if (counter==7 or counter==1) and not no_cord:
                try:
                    yield next(it_cord)
